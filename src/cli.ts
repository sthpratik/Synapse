...
`;
    ...
    console.log(chalk.green('✅ Created synapse.yml configuration file'));
    console.log(chalk.blue('💡 Edit the file and run: synapse run'));
  });
...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.log(chalk.green(`✅ K6 script generated: ${options.output}`));
    } catch (error) {
      console.error(chalk.red('❌ Error:'), error instanceof Error ? error.message : error);
      process.exit(1);
    }
